...
class Dense(nn.Module):
    ...
    def forward(self,x):
        ...
        # original_pose = self.softmax(self.pose_output(x))
        # pose = torch.Tensor(np.array([np.array(original_pose[:,0])]))
        # #pose = pose_converter(original_pose)
        # affinity = self.affinity_output(x) 

        original_pose = self.pose_output(x)
        pose_softmax = self.softmax(original_pose)
        pose = torch.Tensor(pose_softmax)[:,1]
        affinity = self.affinity_output(x)
        
        return (x, pose, affinity, original_pose)
